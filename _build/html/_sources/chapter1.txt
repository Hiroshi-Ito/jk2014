...
ログイン
----
#. Linuxというアイコンをダブルクリック(マウスの左ボタンを連続して2回押す)してください。図のようなユーザ認証画面がでてきます。

    .. figure:: chapter1_img/x-usermei-nyuryoku.png
     :width: 70%
     :align: center
...
#. **パスワード** を入力して下さい。パスワードは秘密のコードです。決して他人に教えてはいけません [#login1]_ 。画面にはアスタリスク記号（＊）が入力した文字数だけ表示されます。途中で間違えたら、``バックスペース`` キーで間違えた部分を消去して、入力し直してください。
#. ``ログオン`` ボタンをクリックしてください [#login2]_ 。
...
これを利用することになります。
    ...
    .. figure:: chapter1_img/login-shoki.png
     :width: 70%
     :align: center
...
.. rubric:: 脚注
.. [#login1] パスワードは定期的に変更することをおすすめします（:ref:`passwd` 参照)。
.. [#login2] ログインとログオン、ログアウトとログオフは同じ意味と考えて問題ありません。UNIXでは通常システムの利用開始・終了手続きをそれぞれログイン・ログアウトと呼び、Windowsではログオン・ログオフと呼びます。
...
ポインタを「システムツール」に移動させると図1.3のようなメニューが現れます。
マウスをドラッグして、「端末」を選択します。ウィンドウが開きます。
ポインタは画面の背景(バックグラウンド：ルートウィンドウ)にいくと``矢印"になり、
ウィンドウの中にあると ``I" になります。

  .. figure:: chapter1_img/tanmatu-hiraku.png
   :width: 30%
   :align: center

   **図 1.3**: 「アプリケーション」のメニュー


  .. figure:: chapter1_img/gnome-tanmatu.png
   :width: 70%
   :align: center
...
3.アクティブなウィンドウ [#X1]_ は、今開けたウィンドウですが、
...
「システム」のメニューは図1.5のような内容になっています。
  ...
  .. figure:: chapter1_img/logout-menu.png
   :width: 30%
   :align: center
...
基本的には3.3.1節の図3.1に示すように木構造である。

*マルチタスク: 同時に複数のプロセス [#unix1]_ が実行できます。
...
.. rubric:: 脚注
.. [#unix1] プロセスとは、プログラムを実行する時に、メモリ上に読み込まれて、CPUが実行できる状態になったデータを指します。したがって、一つのプログラムで複数のプロセスが存在することがあります。
